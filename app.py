...
st.set_page_config(page_title="Matching Inteligente de Vagas", layout="wide")
...
with col1:
  st.caption(
      "Plataforma corporativa para análise estratégica de aderência entre"
      " colaboradores e oportunidades internas."
  )
...
def rol_compativel(rol_colab, rol_vaga):
  colab_r = parse_rol(rol_colab)
  vaga_r = parse_rol(rol_vaga)

  if colab_r["tipo"] != vaga_r["tipo"]:
    return False

  return colab_r["nivel"] == vaga_r["nivel"]
...
def gerar_excel_massivo(df_massivo, df_sem_vagas):
  ...
  with pd.ExcelWriter(output, engine="openpyxl") as writer:
    if not df_massivo.empty:
      df_massivo.to_excel(
          writer, index=False, sheet_name="Matching Massivo"
      )
      aplicar_estilo_excel(writer, "Matching Massivo")
    if not df_sem_vagas.empty:
      df_sem_vagas.to_excel(
          writer, index=False, sheet_name="Colaboradores Não Encontrados"
      )
...
def calcular_matching_colaborador(
    perfil_row,
    vagas,
    colunas_mapa,
    texto_cv="",
  ...
  if status_text:
    status_text.markdown(
        "⚙️ **Processando Análise:** Extraindo dados do perfil e aplicando"
        " filtros de cargo/rol..."
    )
    time.sleep(0.15)
  ...
  descricao_colab = (
      limpar_texto_modelo(perfil_row.get(coluna_descricao, ""))
      if coluna_descricao
      else ""
  )
  nome_perfil = (
      limpar_texto_modelo(perfil_row.get(coluna_nome_perfil, ""))
  ...
  taxa_colab = (
      tratar_taxa(perfil_row.get(coluna_taxa_colab))
  ...
  def filtro_vaga(row):
    if coluna_rol_colab and coluna_rol_vaga:
      if not rol_compativel(
          perfil_row.get(coluna_rol_colab), row.get(coluna_rol_vaga)
      ):
        return False
  ...
  if status_text:
    status_text.markdown(
        "⚙️ **Processando Análise:** Calculando similaridade semântica"
        " (TF-IDF)..."
    )
    time.sleep(0.15)
  ...
  if status_text:
    status_text.markdown(
        "⚙️ **Processando Análise:** Cruzando requisitos detalhados e gerando"
        " breakdown..."
    )
    time.sleep(0.1)
  ...
  for i, row_texto in enumerate(vagas_filtradas["texto"]):
    ...
    perfil_resumido = normalizar_cargo(
        str(row_vaga_i.get("perfil solicitado resumido", ""))
    )
    ...
    rol_c = (
        str(perfil_row.get(coluna_rol_colab, "")) if coluna_rol_colab else ""
    )
    ...
    taxa_v_loop = (
        tratar_taxa(row_vaga_i.get(coluna_taxa_vaga)) if coluna_taxa_vaga else 0
    )
...
if file_vagas and file_colab:
  ...
  if "localidade_municipio" in colab.columns:
    rename_cols["localidade_municipio"] = "Localidade Municipio Colaborador"
  if rename_cols:
  ...
  target_col = (
      col_lugar_def if col_lugar_def else "lugar de trabajo definitivo vaga"
  )
  ...
  if not coluna_nome:
    ...
    st.markdown(
        ...
        f"Selecione manualmente qual coluna representa o <b>nome do"
        f" colaborador</b>:"
        f"</div>",
        unsafe_allow_html=True,
    )
    ...
    coluna_nome = st.selectbox(
        "Qual coluna é o nome do colaborador?", options=colab.columns.tolist()
    )
  ...
  coluna_nome_perfil = encontrar_coluna(
      colab,
      [
          "nome_perfil",
          "perfil",
  ...
  coluna_taxa_colab = encontrar_coluna(
      colab, ["taxa", "tasa", "rate", "valor"]
  )
  coluna_rol_vaga = encontrar_coluna(
      vagas, ["rol reporting", "rol", "role", "nivel"]
  )
  coluna_taxa_vaga = encontrar_coluna(
      vagas,
      [
  ...
  if col_loc_estado and col_loc_estado in colab.columns:
    colab["Localidade Estado Colaborador"] = (
        colab[col_loc_estado].fillna("-").astype(str)
    )
  else:
    colab["Localidade Estado Colaborador"] = "-"

  col_loc_mun = encontrar_coluna(
      colab,
      [
  ...
  if col_loc_mun and col_loc_mun in colab.columns:
    colab["Localidade Municipio Colaborador"] = (
        colab[col_loc_mun].fillna("-").astype(str)
    )
  else:
    colab["Localidade Municipio Colaborador"] = "-"
  ...
  tipo_analise = st.radio(
      ...
      [
          "Análise Individual (Um colaborador)",
          "Análise Massiva (Todos da base de colaboradores)",
      ],
      horizontal=True,
  )
  ...
  if tipo_analise == "Análise Individual (Um colaborador)":
    ...
    if busca:
      ...
      filtro_id = (
          colab["ID"].astype(str).str.contains(busca, case=False, na=False)
      )

      if coluna_matricula:
        filtro_matricula = (
            colab[coluna_matricula].astype(str).str.contains(busca, na=False)
        )
    ...
    selecionado = st.selectbox(
        "Selecione o colaborador", filtro_df[coluna_nome].tolist()
    )
    ...
    st.markdown(
        "<div class='cv-box'><b>Currículo de "
        + str(selecionado)
        + " (Opcional)</b><br><span style='color:#8b949e;font-size:13px;'>"
        + "Anexe o CV em PDF para enriquecer o matching com skills,"
    ...
    cv_pdf = st.file_uploader(
        "Anexar CV em PDF", type=["pdf"], key=f"cv_{selecionado}"
    )
    ...
    if cv_pdf:
      ...
      if texto_cv.strip():
        st.success(
            f"CV de {selecionado} carregado — {len(texto_cv.split())} palavras"
            " extraídas"
        )
      else:
    ...
    if not perfil_texto.strip():
      st.warning(
          "Este colaborador não possui descrição de perfil nem CV anexado. O"
          " match pode ter baixa precisão."
      )
    ...
    if st.button("Buscar Vagas Compatíveis"):
      ...
      score_medio = (
          round(resultado["match"].mean() * 100, 1) if len(resultado) > 0 else 0
      )
    ...
    st.info(
        "O sistema analisará a aderência de **TODOS** os colaboradores. Se o"
        " colaborador não possuir vagas com score >= 50%, todas as vagas"
        " compatíveis serão exibidas como opção."
    )
    ...
    if st.button("Executar Análise Massiva"):
      ...
      status_massivo.markdown(
          "⚙️ **Processando Análise:** Preparando bases e iterando sobre os"
          " colaboradores..."
      )
      time.sleep(0.15)
      ...
      for idx, (_, colab_row) in enumerate(colab.iterrows()):
        ...
        status_massivo.markdown(
            f"⚙️ **Processando Análise ({idx+1}/{total_colab}):** Analisando"
            f" colaborador {colab_row.get(coluna_nome, f'Colab {idx+1}')}..."
        )
        time.sleep(0.01)
        ...
        vagas_matching = calcular_matching_colaborador(
            colab_row, vagas, colunas_mapa
        )
        ...
        if vagas_matching.empty:
          list_sem_vagas.append({
              ...
              "Taxa Colaborador": (
                  colab_row.get(coluna_taxa_colab, "-")
                  if coluna_taxa_colab
                  else "-"
              ),
          ...
          for rank, (_, vaga_row) in enumerate(vagas_finais.iterrows(), 1):
            ...
            registro = {
                ...
                "Tasa Máxima Deseable": vaga_row.get(
                    "tasa máxima deseable", "-"
                ),
                "Perfil Profesional": vaga_row.get("perfil profesional", "-"),
                "Perfil Solicitado Resumido": vaga_row.get(
                    "perfil solicitado resumido", "-"
                ),
                "Lugar de Trabajo Vaga": vaga_row.get(
                    "lugar de trabajo vaga", "-"
                ),
                target_col: vaga_row.get(target_col, "-"),
                "País": vaga_row.get("País", "-"),
                "Perfil Solicitado Detallado": vaga_row.get(
                    "perfil solicitado detallado", "-"
                ),
                "Conocimientos Funcionales": vaga_row.get(
                    "conocimientos funcionales", "-"
                ),
                "Conocimientos Tecnicos": vaga_row.get(
                    "conocimientos tecnicos", "-"
                ),
                "Observaciones Necesidad": (
                    vaga_row.get(col_obs, "-")
                    if col_obs
                    else vaga_row.get("observaciones necesidad", "-")
                ),
                "Outros": vaga_row.get("outros", "-"),
            }
            lista_resultados.append(registro)

      status_massivo.empty()
      progress_massivo.empty()

      st.session_state.resultado_massivo_cache = (
          pd.DataFrame(lista_resultados) if lista_resultados else pd.DataFrame()
      )
      st.session_state.colab_sem_vagas_cache = (
          pd.DataFrame(list_sem_vagas) if list_sem_vagas else pd.DataFrame()
      )

  if tipo_analise == "Análise Massiva (Todos da base de colaboradores)":
    df_massivo = (
        st.session_state.resultado_massivo_cache
        if st.session_state.resultado_massivo_cache is not None
        else pd.DataFrame()
    )
    df_sem_vagas = (
        st.session_state.colab_sem_vagas_cache
        if st.session_state.colab_sem_vagas_cache is not None
        else pd.DataFrame()
    )
    ...
    if not df_massivo.empty or not df_sem_vagas.empty:
      ...
      aba_principal, aba_sem_vagas = st.tabs(
          ["🎯 Lista Geral de Matching", "⚠️ Colaboradores Não Encontrados"]
      )
      ...
      with aba_sem_vagas:
        if not df_sem_vagas.empty:
          st.markdown(
              "Abaixo estão listados os colaboradores para os quais o sistema"
              " não encontrou nenhuma vaga alinhada nos filtros iniciais."
          )
          st.dataframe(df_sem_vagas, use_container_width=True, height=400)
        else:
          st.success(
              "Todos os colaboradores tiveram pelo menos uma vaga compatível"
              " encontrada!"
          )
  ...
  if (
      st.session_state.resultado_cache is not None
      and tipo_analise == "Análise Individual (Um colaborador)"
  ):
    ...
    if "match" in resultado_exibicao.columns:
      resultado_exibicao["Match Score (%)"] = (
          resultado_exibicao["match"] * 100
      ).round(2).astype(str) + "%"
    ...
    if busca_atual:
      vagas_detalhe = resultado[
          resultado["necesidad"]
          .astype(str)
          .str.contains(busca_atual, case=False, na=False)
      ]
      if vagas_detalhe.empty:
        st.warning(
            f"Nenhuma vaga encontrada com a necessidade '{busca_atual}'."
        )
      ...
      with st.expander(titulo, expanded=True):
        ...
        **Score Match:** {round(row['match'] * 100, 2)}%
        ...
        st.markdown(
            ...
            " margin-bottom:20px;'>Cada critério abaixo mostra quanto"
            " contribuiu para o score final. Critérios em vermelho ou laranja"
            " indicam o que faltou para chegar a 100%.</p>",
            unsafe_allow_html=True,
        )
        ...
        def _barra(pct, cor):
          ...
          return (
              f"<div"
              f" style='background:#21262d;border-radius:8px;height:14px;width:100%;overflow:hidden;'><div"
        ...
        def _linha(
          ...
          cor_status = (
              "#238636"
              if ok
              else ("#f0883e" if pct_exibir > 0 else "#f85149")
          )
        ...
        rol_colab_val = (
            str(
                colab[colab[coluna_nome] == selecionado]
                .iloc[0]
                .get(coluna_rol_colab, "")
            )
            if coluna_rol_colab
            else ""
        )
        ...
        perfil_prof_vaga = (
            str(row.get("perfil solicitado resumido", "")).strip() or "—"
        )
        ...
        faltou_cargo = (
            ...
            if cargo_ok
                ...
                f"O cargo '{nome_perfil_val}' não foi encontrado nos requisitos"
                " da vaga."
            )
        )
        breakdown_html += _linha(
        ...
        faltou_rol_nivel = (
            ...
            if rol_ok
                ...
                f"A vaga exige o nível '{rol_vaga_val or '—'}', mas o"
                f" colaborador é '{rol_colab_val or '—'}'. Os níveis"
                " precisam ser iguais para pontuar."
            )
        )
        breakdown_html += _linha(
        ...
        if not taxa_c and not taxa_v:
          ...
          faltou_taxa = (
              f"A taxa do colaborador ({taxa_c}) é maior que o limite da vaga"
              " ({taxa_v}). Essa vaga seria descartada na análise."
          )
        ...
        faltou_cv = (
            ...
            if cv_ok
                ...
                "O CV ainda não foi enviado. Anexe o PDF do colaborador para"
                " melhorar o resultado do matching."
                if not cv_presente
                else (
                    "O CV foi enviado, mas tem poucas palavras em comum com os"
                    " requisitos dessa vaga."
                )
            )
        )
        cv_label = (
            "CV enviado e considerado na análise"
            if cv_presente
            else "CV não enviado"
        )
        breakdown_html += _linha(
        ...
        cor_total = (
            "#238636"
            if score_pct >= 70
            else ("#f0883e" if score_pct >= 50 else "#f85149")
        )
        cor_status = cor_total
        nota_score = (
            "Ótima aderência — colaborador muito compatível com a vaga."
            if score_pct >= 70
            else (
                "Aderência moderada — vale avaliar os critérios em"
                " laranja/vermelho."
                if score_pct >= 50
                else (
                    "Baixa aderência — o colaborador tem pouco em comum com"
                    " essa vaga."
                )
            )
        )
    ...
    if "match" in resultado_excel.columns:
      resultado_excel["Match Score (%)"] = (
          resultado_excel["match"] * 100
      ).round(2).astype(str) + "%"
...
st.markdown(
    "<div class='footer-wrapper'><div class='footer-box'><div"
    " class='footer-title'>Matching Inteligente de Vagas</div><div"
    " class='footer-description'>Plataforma corporativa de apoio estratégico"
    " para análise de aderência entre colaboradores e oportunidades internas,"
    " utilizando IA, Skills, Perfil Profissional e Currículo PDF.</div><div"
    " class='footer-author'>Desenvolvido por <b>Jonathan Marquezini</b> | UGR"
    " Brasil</div></div></div>",
    unsafe_allow_html=True,
)
